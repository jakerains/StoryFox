...
struct IOSGenerationProgressView: View {
    ...
    private var horizontalPadding: CGFloat {
        sizeClass == .compact
            ? StoryJuicerGlassTokens.Spacing.medium
            : StoryJuicerGlassTokens.Spacing.xLarge + 8
    }
    ...
    private var imageProgressGrid: some View {
        let minSize: CGFloat = sizeClass == .compact ? 90 : 120

        return GlassEffectContainer(spacing: StoryJuicerGlassTokens.Spacing.small) {
            LazyVGrid(
                columns: [GridItem(.adaptive(minimum: minSize, maximum: 160), spacing: StoryJuicerGlassTokens.Spacing.small)],
